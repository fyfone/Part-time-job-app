...
public class CompanyInviteRecordActivity extends BaseActivity implements
		IXListViewListener {
...
//		jlList.setOnItemClickListener(new ListViewItemClickListener());
		inviteRecordAdapter = new CompanyInviteRecordAdapter();
		loadData();
		// initApplyRecordView();
	}

	private List<CompanyInvieRecord> applyRecords;

	private void loadData() {

		// 获取企业列表
		Map<String, String> params = new HashMap<String, String>();
		params.put(Globals.WS_POST_KEY, "{\"Ac\":\"FIN\",\"Para\":{\"Sid\":\""
				+ User.sessionId + "\",\"P\":\"" + page + "\",\"I\":\"" + id
				+ "\"}}");
		new VolleyUtil() {

			public void analysisData(String response) {
				CompanyInviteRecordResult s = GsonUtils.json2bean(response,
						CompanyInviteRecordResult.class);
				if (s == null || !(s.Stu == 1)) {
					Toast.makeText(CompanyInviteRecordActivity.this,
					...
					if (page == 1 && s.Rst.Lst.size() == 0) {
						...
						if (page == 1) {
							...
							List<CompanyInvieRecord> js = s.Rst.Lst;
							for (int i = 0; i < js.size(); i++) {
								applyRecords.add(js.get(i));
							}
						}
						inviteRecordAdapter.getDate(CompanyInviteRecordActivity.this,
								applyRecords);
						if (page == 1) {
		...
		}.volleyStringRequestPost(CompanyInviteRecordActivity.this, params);
	...
	private void initApplyRecordView() {
		if (applyRecords != null && applyRecords.size() > 0) {
			inviteRecordAdapter
					.getDate(CompanyInviteRecordActivity.this, applyRecords);
			jlList.setAdapter(inviteRecordAdapter);
			inviteRecordAdapter.notifyDataSetChanged();
		} else {
			Toast.makeText(CompanyInviteRecordActivity.this, R.string.job_no_info,
					Globals.TOAST_SHORT);
		}
	}
...
//			intent.setClass(CompanyInviteRecordActivity.this,
