...
public class MyMsgInvitationAdapter extends BaseAdapter {

	private List<InvieRecord> dataSet;
	private LayoutInflater mInflater;
	private Context context;

	public MyMsgInvitationAdapter() {

	}

	public void getDate(Context context, List<InvieRecord> dataSet) {
	...
	public View getView(final int location, View convertView, ViewGroup parent) {
		...
		holder.sqjl_item_loca
				.setText(dataSet.get(location).JE + "  ("
						+ dataSet.get(location).S + "/"
						+ dataSet.get(location).U + ")");
		holder.tv_messg.setText(dataSet.get(location).CE);
		holder.tv_msg_invit_time.setText(dataSet.get(location).IE);
		holder.tv_msg_red.setText("(" + dataSet.get(location).IST + ")");
