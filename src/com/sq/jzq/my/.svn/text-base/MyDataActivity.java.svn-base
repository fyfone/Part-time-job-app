			...
			my_titlebar
					.setClickEnterButtonListener(new OnClickEnterButtonListener() {
						...
						public void onClickEnterButton(View v) {
							...
							if(!et_my_ems.getText().toString().trim().matches("\\w+@\\w+\\.\\w+")) {
								Toast.makeText(MyDataActivity.this,
										"您输入的邮箱格式不正确！", Globals.TOAST_SHORT)
										.show();
								

							}
							else if (!et_my_contact.getText().toString().trim().equals("") && et_my_contact.getText().toString().trim().length() != 11) {
//								if(et_my_contact.getText().toString().trim().length() != 11) {
									Toast.makeText(MyDataActivity.this,
											"您输入的联系方式格式不正确！", Globals.TOAST_SHORT)
											.show();
									
//								}

							} else if(!AppUtil.isID(et_my_id_number.getText().toString().trim())) {
								Toast.makeText(MyDataActivity.this,
										"您输入的身份证号格式不正确！", Globals.TOAST_SHORT)
										.show();
